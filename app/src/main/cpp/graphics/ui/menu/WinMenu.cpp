...
void WinMenu::initGraphics(float maxX, float maxY, GLuint vertexHandle, GLuint textureHandle){
	...
	float excellentX = (maxX  - Label::getWidthForHeight(excellentHeight)*std::strlen(excellent)) / 2.0;
	...
	float highScoreX = (maxX - Label::getWidthForHeight(highScoreHeight)*std::strlen(HIGH_SCORE_FORMAT_STRING)) / 2.0;
